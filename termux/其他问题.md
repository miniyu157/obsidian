...
```bash
paru -S --noconfirm yaru-xfwm4-theme yaru-gtk-theme yaru-icon-theme
```

qogir

```bash
paru -S --noconfirm qogir-gtk-theme qogir-icon-theme
```

mint-y （无 xhdpi 标题栏，推荐使用图标）

```bash
paru -S --noconfirm mint-themes mint-y-icons 
```

Nordic-xhdpi

```bash
sudo git clone https://github.com/yumagulovrn/Nordic-xhdpi.git /usr/share/themes/Nordic-xhdpi && xfconf-query -c xfwm4 -p /general/theme -s Nordic-xhdpi && paru -S --noconfirm nordic-theme nordic-darker-theme papirus-icon-theme papirus-folders-nordic
```
...
### 某一次滚更新后出现了 tty: ttyname error: No such device

修复它

```bash
sudo sed -i.bak 's#case $( /usr/bin/tty ) in#case $( /usr/bin/tty 2>/dev/null ) in#' /etc/profile.d/gpm.sh
```