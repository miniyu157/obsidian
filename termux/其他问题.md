...
>
> ```bash
> echo 'set -gx QT_SCALE_FACTOR 2' >> ~/.config/fish/config.fish
> ```

让窗口边框也可以缩放 & 推荐主题

```bash
sudo pacman -S yaru-xfwm4-theme yaru-gtk-theme yaru-icon-theme
```
...
```bash
sudo pacman -S ffmpegthumbnailer libgepub libgsf poppler-glib libopenraw
```
